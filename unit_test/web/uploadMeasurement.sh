...
if [ "$1" != 2 ]; then
    ...
    printf "Use the command: ./webServices.sh <HOST_IP> <PORT>\n"
    exit 1;
fi
...
url='http://'$1':'$2'/ngsi10/updateContext'
printf "Uploading a measurement for the new user...\n"
printf "POST $url\n"
curl -vX POST $url -d @measurement.json -H "Accept: application/json" -H "Content-Type: application/json"
printf "\n"
